...
def queue_email(subject, recipients, body):
    # Always queue the email, even if not fully configured (allows fallback file generation)
...
def send_email_now(subject, recipients, body):
    ...
    if current_app.config.get("TESTING") or not mail_is_configured():
        # Fallback to local files in development/testing
        sent_emails_dir = os.path.join(current_app.instance_path, "sent_emails")
        os.makedirs(sent_emails_dir, exist_ok=True)
        filename = f"{utcnow().strftime('%Y%m%d%H%M%S')}_{subject.replace(' ', '_').replace(':', '_')}.txt"
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            f.write("----------------------------------------\n")
            f.write(body)
        current_app.logger.info("Saved local copy of email to %s", filepath)
        return
...
def queued_email_batch(limit=None):
    ...
    if engine is not None and engine.dialect.name in {"postgresql", "mysql"}:
...
def process_email_queue(limit=None):
    ...
    batch = queued_email_batch(limit=limit)
    for queued_email in batch:
        # Atomic double-check status to prevent multi-process duplicate execution
        if queued_email.status not in (EmailStatus.QUEUED.value, EmailStatus.FAILED.value):
            continue
